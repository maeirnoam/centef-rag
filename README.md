...
# 2. Populate manifest from generated summaries (automatic metadata extraction)
python tools/populate_manifest.py

# 3. Create a new Vertex AI Search datastore for summaries
#    Point it to gs://centef-rag-chunks/summaries/**/*.jsonl

# 4. Create a Search App that combines both datastores
#    Update SERVING_CONFIG to point to the App's serving config

# 5. Searches now automatically query both summaries and chunks
python tools/search_with_summary.py "What are the terrorist financing methods?"
```

### Manifest Format
The manifest is automatically populated from summaries using `tools/populate_manifest.py`:
```json
{
  ...
  "document_type": "pdf",
  "language": "en",
  "source_uri": "gs://centef-rag-bucket/data/document.pdf",
  "chunks_uri": "gs://centef-rag-chunks/data/document.pdf.jsonl",
  "summary_uri": "gs://centef-rag-chunks/summaries/doc.jsonl",
  "num_chunks": 27,
...
**Required fields**: `source_id`, `title`, `document_type`, `language`, `source_uri`, `chunks_uri`, `summary_uri`, `num_chunks`  
**Optional fields**: `author`, `speaker`, `organization`, `date`, `tags`

The manifest serves as a single source of truth for all document metadata, automatically extracted from the summaries generated by Gemini.
